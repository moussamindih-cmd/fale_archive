...
class ReportExportService {
  ...
  static Future<Uint8List> generateArchivesReportPdf({
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (context) =>
            _buildHeader('BORDEREAU RÉCAPITULATIF DES ARCHIVES JOURNALIÈRES'),
  ...
  static Future<Uint8List> generateLogisticsReportPdf({
    ...
    final validCount = items
        .where((i) => i.status == LogisticsStatus.valide)
        .length;
    final pendingCount = items
        .where((i) => i.status == LogisticsStatus.enAttente)
        .length;
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.all(32),
        header: (context) =>
            _buildHeader('ÉTAT DE SYNTHÈSE LOGISTIQUE & FACTURES'),
        footer: (context) => _buildFooter(context, generatedBy),
        build: (context) => [
          pw.SizedBox(height: 12),
          _buildMetaBlock([
            ...
            data: items
                .map(
                  (i) => [
                    ...
                    i.supplier,
                    i.amount != null
                        ? '${_currencyFormat.format(i.amount)} F'
                        : '—',
                    i.status.label,
  ...
  static Future<Uint8List> generateCandidatesReportPdf({
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.all(32),
        header: (context) =>
            _buildHeader('FICHE RÉCAPITULATIVE DU VIVIER DE CANDIDATS RH'),
        footer: (context) => _buildFooter(context, generatedBy),
        build: (context) => [
          pw.SizedBox(height: 12),
          _buildMetaBlock([
  ...
  static String generateLogisticsCsv(List<LogisticsItem> items) {
    ...
    buffer.writeln(
      'Type;Reference;Fournisseur;Montant_FCFA;Statut;Date_Emission;Enregistre_Par;Validateur',
    );
        ...
        children: [
          pw.Column(
            ...
            children: [
              ...
              pw.Text(
                'Système de Gestion Électronique de Documents & Traçabilité',
  ...
  static pw.Widget _buildFooter(pw.Context context, String user) {
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          pw.Text(
            'Document certifié conforme — FALE Archives',
