...
enum ReportType {
  archives(
    ...
    'Bordereau officiel de versement et conformité documentaire',
    Icons.folder_open_rounded,
    Color(0xFF2563EB),
  ),
  logistics(
    'Logistique & Dépenses',
    'Synthèse des factures, justificatifs et statuts de validation',
    Icons.receipt_long_rounded,
    Color(0xFF8B5CF6),
  ),
  candidates(
    'Vivier Candidats RH',
    'Récapitulatif des candidatures, qualifications et étapes de recrutement',
        ...
        case ReportType.logistics:
          {
            ...
            final pdfBytes =
                await ReportExportService.generateLogisticsReportPdf(
                  items: list,
                  generatedBy: userName,
                  startDate: startDate,
                  endDate: endDate,
            ...
            await Printing.layoutPdf(
              onLayout: (_) => pdfBytes,
              name:
                  'Rapport_Logistique_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
            );
            break;
          }
        ...
        case ReportType.candidates:
          {
            ...
            final pdfBytes =
                await ReportExportService.generateCandidatesReportPdf(
                  candidates: list,
                  generatedBy: userName,
                );
            await Printing.layoutPdf(
              onLayout: (_) => pdfBytes,
              name:
                  'Vivier_Candidats_RH_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf',
            );
            break;
          }
      }
